  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/predict", {
        category,
        text,
      }); // Send the category and text in the request body
      setClassification(response.data.classification);
    } catch (error) {
      console.error("Error:", error);
    }
  ...
  return (
    ...
    <div className={`${style.wrapper}`}>
      <div className={`${style.container}`}>
        ...
        {error && <p className="error-message">{error}</p>}
        {classification === "Fake" && (
          <h5 className={`${style.showclass}`}>
            Classification:{" "}
            <h5 className={`${style.determine}`}>{classification}</h5>
          </h5>
        )}
        {classification === "Real" && (
          <h5 className={`${style.showclass}`}>
            Classification:{" "}
            <h5 className={`${style.determine1}`}>{classification}</h5>
          </h5>
        )}
      </div>
    </div>
